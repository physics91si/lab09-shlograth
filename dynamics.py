...
def time_step(dt, mol):
    ...
    force = mol.get_force()
    mol.p1.vel += (dt/2) * force/(mol.p1.m)     
    mol.p2.vel += (dt/2) * -force/(mol.p2.m)
    mol.p1.pos += dt * mol.p1.vel
    mol.p2.pos += dt * mol.p2.vel
    '''mol.p1.acc = (mol.get_force/mol.m1) * dt
    mol.p2.acc = (mol.get_force/mol.m2) * dt
    mol.p2.vel += (dt/2) * force/(mol.p2.m)
    mol.p1.vel += (dt/2) * force/(mol.p1.m)'''
